...
const usedCarLoanHero = "/assets/used-car-loan-hero.png";
import {
  Accordion,
  AccordionContent,
  AccordionItem,
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-[hsl(var(--purple-deep))] text-white">
                    ...
                    <th className="text-right px-4 py-3 font-semibold text-xs">Action</th>
                  </tr>
                </thead>
                <tbody>
                  {interestRates.map((row, i) => (
                    <tr key={row.lender} className={`${i % 2 === 0 ? "bg-card" : "bg-muted/30"} border-t border-border`}>
                      ...
                      <td className="px-4 py-3 text-right">
                        <Link to="/login" data-testid={`apply-now-${row.lender.toLowerCase().replace(/\s+/g, '-')}`} className="inline-flex items-center px-3 py-1.5 rounded-lg bg-primary text-primary-foreground text-xs font-semibold hover:opacity-90 transition-opacity whitespace-nowrap">
                          Apply Now
                        </Link>
                      </td>
                    </tr>
