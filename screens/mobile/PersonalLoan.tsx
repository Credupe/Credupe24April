          ...
          <div className="lg:col-span-5 flex flex-col space-y-6 text-left order-2 lg:order-1 mt-6 lg:mt-0">
            ...
            <div className="hidden lg:block">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-violet-50 text-violet-700 text-xs font-bold rounded-full mb-3 border border-violet-100">
                <ShieldCheck className="w-3.5 h-3.5" /> Instant Eligibility Check
              </span>
              <h1 className="text-3xl font-black text-slate-900 tracking-tight leading-tight">
                {lender ? `${lender.name} Personal Loan` : "Personal Loan Application"}
              </h1>
              <p className="text-sm text-slate-500 mt-2 leading-relaxed">
                Apply online in minutes. Get pre-approved offers from 80+ trusted bank & NBFC partners with minimal documentation.
              </p>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 gap-3.5">
                {[
                  { title: "Lowest Interest Rates", desc: "Starting at just 10.49% p.a. from premium lenders." },
                  { title: "Zero Collateral Required", desc: "Unsecured personal loans without pledging assets." },
                  { title: "Quick & Simple Process", desc: "Completely digital verification & instant approval status." },
                  { title: "Flexible Repayment", desc: "Select comfortable tenure up to 60 months." }
                ].map((item, idx) => (
