...
type Props = NativeStackScreenProps<RootStackParamList, "ApplyEducationLoan">;
...
export const ApplyEducationLoanScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const handleCopyUTM = async (bank: Lender) => {
    ...
    const utmLink = `https://credupe.com/mobile/education-loan?bank=${bank.slug}&partner=${partnerCode}`;
    ...
    try {
      ...
      if (success) {
        ...
        await Share.share({
          message: `Apply for Education Loan with ${bank.name} here: ${utmLink}`,
  ...
  const validateForm = (): boolean => {
    ...
    } else if (!/^\d{5}\d{4}[A-Z]{1}$/i.test(pan) && !/^[A-Z]{5}\d{4}[A-Z]{1}$/i.test(pan)) {
      // standard regex for PAN card
      ...
      <View style={[styles.container, { backgroundColor: colors.bg }]}>
        {!selectedBank ? (
          ...
          loadingLenders ? (
            ...
            >
              <Text style={[styles.listTitle, { color: colors.textMuted }]}>Choose Bank for Education Loan</Text>
              ...
              <View style={[styles.formContainerCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                ...
                <View style={styles.cardBrandHeader}>
                  <Text style={[styles.applicationTitle, { color: colors.text, marginLeft: 0 }]}>
                    {selectedBank.name} Education Loan Application
                  </Text>
                </View>
